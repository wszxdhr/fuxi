...
export async function viewPr(branch: string, cwd: string, logger: Logger): Promise<GhPrInfo | null> {
  const result = await runCommand('gh', ['pr', 'view', branch, '--json', 'number,title,url,state,headRefName'], { cwd });
...
export async function listFailedRuns(branch: string, cwd: string, logger: Logger): Promise<GhRunInfo[]> {
  const result = await runCommand('gh', ['run', 'list', '--branch', branch, '--json', 'databaseId,name,status,conclusion,url', '--limit', '5'], { cwd });
  if (result.exitCode !== 0) {
